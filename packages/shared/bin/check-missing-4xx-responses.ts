...
Note: 500 系のチェックは意図的にデフォルト対象外にしている。監視系の
operation (/healthz, /readyz 等) に 500 (InternalServerError) レスポンス
定義の欠落があり (--status=400,500 --methods=get,post,put で確認可能)、
その解消は本ツール導入時のスコープ外だったため。`)
}
...
function main() {
    ...
    .map((op) => {
      return {
        ...
        missingStatuses: targetStatuses.filter((status) => {
          ...
          // parameters) を表す。hub では `validator('param')` /
          // `validator('json')` の失敗でも 400 を返すため、requestBody を
          ...
          if (status === '400' && !op.hasRequestBody && !op.hasParameters) {
            return false
          }
  ...
  console.log(`Total ${targetMethods.join('/')} operations: ${targetOperations.length}`)
  console.log(`Missing ${targetStatuses.join('/')}: ${missing.length} operations`)
  console.log('---')
